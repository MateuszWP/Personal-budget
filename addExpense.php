<?php
	...
	if (isset($_POST['amount']))
	{
		...
		{
			$good=false;
			$_SESSION['e_payingMethod']="Wybierz metode płatności";
		}
		else
		{
			$payingMethod = $_POST['payingMethod'];
		}
		...
		if($good == true)
		{
			$query = $db->prepare('INSERT INTO expenses VALUES (NULL, :userId, :amount, :date, :payingMethod, :category, :comment)');
			$query->bindValue(':userId', $_SESSION['logged_id'], PDO::PARAM_INT);
			$query->bindValue(':amount', $amount, PDO::PARAM_STR);
			$query->bindValue(':date', $date, PDO::PARAM_STR);
			$query->bindValue(':payingMethod', $payingMethod, PDO::PARAM_STR);
			...
			<div class="col-md-4 offset-md-4  col-8 offset-2  text-center mt-5" id="logowanie">
				...
				<form id="form" method="post">
					...
					<div class="input-group form-group">
						...
						<input class="form-control" type="date" min="2000-01-01" name="date" id="date">
					</div>
					...
					<div class="input-group form-group">
						...
						<select class="form-control" name="payingMethod">
							<option value="" disabled selected hidden>Metoda płatności </option>
					...
					<?php
							if (isset($_SESSION['e_payingMethod']))
							{
								echo '<div class="error">'.$_SESSION['e_payingMethod'].'</div>';
								unset($_SESSION['e_payingMethod']);
							}
					?>
					...
					<div class="input-group form-group">
						...
						<select class="form-control" name="category">
							...
							<option value="e">Na emeryturę</option>
							<option value="sd">Spłata długów</option>
							<option value="dar">Darowizna</option>
